...
class prompt(base):
    def __init__(self, config):
        ...
        self.tokenizer.pad_token = self.tokenizer.sep_token_id
        self.model = GPT2LMHeadModel.from_pretrained('ckiplab/gpt2-base-chinese')
        self.model_demo = GPT2LMHeadModel.from_pretrained('ckiplab/gpt2-base-chinese')
        self.state_network = nn.Sequential(nn.Dropout(0.1), nn.Linear(768, 1))
        self.state_network_demo = nn.Sequential(nn.Dropout(0.1), nn.Linear(768,1))

        if config.model != 'ckiplab/gpt2-base-chinese':
    ...
    def prepare_input(self, task, inputs_id, mask, model):
        ...
        _, past = model(prev_input, past_key_values=None, return_dict=False)
        ...
        prev_input, past = model(inputs_id, past_key_values=past, attention_mask=new_mask, position_ids=position_ids, return_dict=False)
