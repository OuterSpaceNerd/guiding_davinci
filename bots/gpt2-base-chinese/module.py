...
class bot(nn.Module):
    def __init__(self, config):
        ...
        self.lm = GPT2LMHeadModel.from_pretrained("ckiplab/gpt2-base-chinese")
    ...
    def make_response(self,prefix_sentences, prompts):
        ...
        with torch.no_grad():
            ...
            for i in range(len(sentences)):
                tmp = self.tokenizer.encode(sentences[i])
            ...
            _, past = self.lm(prev_input, past_key_values=None, attention_mask=m, position_ids=position_ids, return_dict=False)
            ...
            for i in range(128):
                prev_input, past = self.lm(prev_input, past_key_values=past, attention_mask=m, position_ids=position_ids, return_dict=False)
                m = torch.cat((m, append), 1)
                position_ids = m.long().cumsum(-1) - 1
                position_ids.masked_fill_(m == 0, 1)
                position_ids = position_ids[:, -1].unsqueeze(-1).to(self.device)
                ...
                prev_input = torch.softmax(prev_input[:, :-1], dim=-1)
            ...
            # print(temp_sen)
            a = self.tokenizer.batch_decode(temp_sen, skip_special_token=False)
            # print(len(a))
